...
Plugin URI: http://opentuteplus.com/wordpress-post-contributors/
...
class WP_Post_Contributors {
	...
	public function render_meta_box_content( $post ) {
		...
		echo '</label> ';
		echo '<br /> ';
		echo '<select name="post_contributors_list[]" multiple size="5">';
		$selected = '';
		...
		foreach ( $blogusers as $bloguser ) {
			...
			echo '<option id="post_contributors_list"';
	        echo ' value="' . esc_attr( $bloguser_id ) . '" size="25" '. $selected . '/>';
...
function load_post_contributors_styles() {
    wp_enqueue_style( 'post-contributors', plugins_url( 'assets/css/wp-post-contributors.css', __FILE__ ) );
}
